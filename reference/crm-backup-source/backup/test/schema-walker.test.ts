...
import { walkSchema } from '../src/schema-walker.js';
import type { ObjectSchema } from '../src/crm/types.js';
...
describe('SchemaWalker', () => {
  ...
  it('handles diamond graph — shared child node visited once, not marked as cycle break', async () => {
    ...
    expect(describeCalls).toBe(4); // Account, Contact, User, Opportunity (User not re-described)
    // The second edge to User (from Opportunity) should NOT be a cycle break
    const userEdges = graph.edges.filter((e) => e.childObject === 'User');
    expect(userEdges).toHaveLength(2); // from Contact and from Opportunity
  ...
  it('does not make redundant describe calls for already-visited objects', async () => {
    ...
    expect(describeCalls).toBe(2); // Account + Contact
  });
});
