...
// ---------------------------------------------------------------------------
// inspect_org — 4 s
// "Reading Salesforce schema… 47 custom objects, 12 GB storage"
// ---------------------------------------------------------------------------

export const inspectOrgTool = betaZodTool({
  name: 'inspect_org',
  description:
  ...
  run: async (_args) => {
    ...
    const output = {
      customObjects: 47,
      storageBytes: 12_884_901_888, // 12 GB
      sobjects: [
        'Account',
        'Contact',
        'Opportunity',
...
// ---------------------------------------------------------------------------
// pick_backend — 7 s
// "GCS chosen — closest region, lowest tier cost"
// ---------------------------------------------------------------------------

export const pickBackendTool = betaZodTool({
  name: 'pick_backend',
  description:
...
// "Encrypted credentials written"
// ---------------------------------------------------------------------------

export const writeStorageConfigTool = betaZodTool({
...
// "Created 4 routing rules from your data shape"
// ---------------------------------------------------------------------------

const RULE_TARGETS = ['gcs:STANDARD', 'gcs:NEARLINE', 'gcs:COLDLINE', 'gcs:ARCHIVE'] as const;

export const generateStarterRulesTool = betaZodTool({
    ...
    const rules = [
      {
        ...
        target: RULE_TARGETS[2], // COLDLINE
        priority: 10,
      ...
      },
      {
        ...
        target: RULE_TARGETS[1], // NEARLINE
        priority: 20,
      ...
      },
      {
        ...
        target: RULE_TARGETS[0], // STANDARD
        priority: 30,
...
export const deploySfPackageTool = betaZodTool({
  ...
  run: async (_args) => {
    ...
    const output = {
      ...
      entityCount: 2, // VastifyFile__x, VastifyRecord__x
      deployedComponents: [
        'ExternalDataSource/Vastify_OData',
...
export const validateConnectionTool = betaZodTool({
...
// ---------------------------------------------------------------------------
// Ordered tool list (used by runner)
// ---------------------------------------------------------------------------
